...
(function (factory) {
  ...
  src_boardWidget_board = function ($, getSiteUrl, getList, getListFormCollection, getListItems, updateListItems, getNodesFromXml, fillTemplate, makeSameHeight, addHoverEffect, boardTemplate) {
    ...
    Board.defaults = {
      ...
      template: null,
      webURL: getSiteUrl(),
      //FIXME: this call needs to be deferred until first usage.
      showColPicker: false,
...
.promise();
  ...
  src_peoplePickerWidget_peoplePicker = function ($, peoplePickerTemplate, getSiteUrl, searchPrincipals, resolvePrincipals, parseLookupFieldValue, addHoverEffect) {
    ...
    People.defaults = {
      ...
      onCreate: null,
      onRemoveUser: null,
      inputPlaceholder: 'Escriba y seleccione usuario',
      appendTo: null,
      minLength: 1,
          ...
          select: function (ev, u) {
            ...
            if (u.item.accountId !== '-1' || !o.resolvePrincipals) {
              ...
              resolvePrincipals({ principalKeys: u.item.accountName }).then(function (xmlDoc) {
                ...
                u.item.accountId = $(xmlDoc).find('AccountName:contains(\'' + u.item.accountName + '\')').parent().find('UserInfoID').text();
                addToSelectionList();
              });
            }
          }
      ...
      if (type === 'method') {
        ...
        case 'remove':
          if (options) {
            ...
            if (!rmEle.length) {
              rmEle = o.eleSelected.find('div[data-pickspusername=\'' + options + '\']');
            }
            if (rmEle.length) {
  ...
  src_filterPanelWidget_filterPanel = function ($, filterPanelTemplate, getSiteUrl, getList, parseLookupFieldValue, fillTemplate, getCamlLogical, xmlEscape, lookupFieldWidget, peoplePickerWidget, dateFieldWidget) {
    ...
    Filter.defaults = {
      list: '',
      webURL: getSiteUrl(),
      // FIXME: this needs to be deferred until first call
      columns: ['Title'],
